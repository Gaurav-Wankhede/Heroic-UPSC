...
export default function MainsPage() {
  ...
  useEffect(() => {
    async function fetchPosts() {
      const response = await fetch('/api/posts?category=Mains');
  ...
  const currentPosts = allRecentPosts
    .filter(post => post.category === 'Mains')
    .slice(indexOfFirstPost, indexOfLastPost);

  const totalPages = Math.ceil(allRecentPosts.filter(post => post.category === 'Mains').length / postsPerPage);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-4xl font-bold mb-8">UPSC Mains Preparation</h1>
      
      <section className="mb-12">
        <h2 className="text-2xl font-semibold mb-4">Mains Categories</h2>
          ...
          {currentPosts.map((post: Post) => (
            <Card key={post.id}>
              ...
              <CardContent>
                ...
                <Link href={`/mains/${post.slug}`} className="text-primary hover:underline">
                  Read more
                </Link>
              </CardContent>
            </Card>
